...
const Zone = () => {
  ...
  const getAllZone = async () => {
    try {
      ...
      if (response.data.tickets != null) {
        ...
        const result = filterByProperty(filterResponse, "ticket_zone", true);
        console.log(result);
        setZone(result);
      } else {
        alert("Ticket-zone list is empty");
      }
      // setZone(response.data.tickets)
    } catch (error) {
      console.error(error);
    }
  };
  ...
  return (
    <div className="h-auto m-[2px] p-[25px] bg-white rounded-[30px] mt-4 flex flex-col gap-[20px] md:h-screen md:overflow-y-auto md:rounded-none md:m-0">
      <AddButton title={"Ticket Zone"} link={"/role/superadmin/zone/add-new"} />
      <div className="w-full p-4 rounded-xl bg-gray-200 flex items-center justify-start">
        <BsSearch />
        <input
          type="text"
          placeholder="Search ticket zone"
        ...
        {searchData ? (
          searchData.length > 0 ? (
            searchData?.map((zone, i) => {
              return (
                <Link
                  to={`/role/superadmin/zone/${zone._id}`}
                  className="w-full"
                  key={i}
                >
                  <InfoButton
