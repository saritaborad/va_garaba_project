  ...
  const getAllZone = async () => {
    try {
      ...
      if (response.data.tickets != null) {
        ...
        const result = filterByProperty(filterResponse, "pass_zone", true);
        setZone(result);
      } else {
        ...
        {searchData ? (
          searchData.length > 0 ? (
            searchData?.map((zone, i) => {
              return (
                <Link
                  to={`/role/superadmin/zone/passzoneid/${zone._id}`}
                  className="w-full"
                  key={i}
                >
                  <InfoButton
